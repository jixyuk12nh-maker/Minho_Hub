...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        -- Cosmetics catalog / selector.
        -- Uses the game's CosmeticLibrary when available; this is only a local catalog/UI layer.
        local Catalog = {}
        ...
        function Catalog:_getLibrary()
            local lib
            pcall(function()
                if KiciaHook and KiciaHook.ao then
                    lib = KiciaHook.ao.CosmeticLibrary
                end
            end)
            if not lib then
                pcall(function() lib = CosmeticLibrary end)
            end
            return lib
        end

        function Catalog:Build()
            local lib = self:_getLibrary()
            local cosmeticsTable = lib and lib.Cosmetics
            for _, t in ipairs({"Skin", "Wrap", "Charm", "Finisher", "Emote"}) do
                table.clear(self._byType[t])
                table.clear(self._lookupByType[t])
            end
            table.clear(self._typeByName)
            if type(cosmeticsTable) ~= "table" then
                self.built:Fire()
                return
            end

            -- If no rarity filter was explicitly supplied, include every rarity present.
            if next(self._raritySet) == nil then
                for _, data in pairs(cosmeticsTable) do
                    if type(data) == "table" then
                        local rarity = rawget(data, "Rarity")
                        if rarity ~= nil then self._raritySet[rarity] = true end
                    end
                end
            end
            ...
            for name, data in pairs(cosmeticsTable) do
                if type(data) == "table" then
                    local t = rawget(data, "Type")
                    if t and self._typeSet[t] and self._byType[t] then
                        self._typeByName[name] = t
                        table.insert(self._byType[t], name)
                        local rarity = rawget(data, "Rarity")
                        if rarity and self._raritySet[rarity] then
                            if t == "Skin" then
                                local itemName = rawget(data, "ItemName")
                                if itemName then
                                    local lookup = self._lookupByType.Skin[itemName]
                                    if not lookup then
                                        lookup = {}
                                        self._lookupByType.Skin[itemName] = lookup
                                    end
                                    lookup[name] = true
                                end
                            else
                                self._lookupByType[t][name] = true
                            end
                        end
                    end
                end
            ...
            for _, list in pairs(self._byType) do
                table.sort(list, function(a, b) return tostring(a):lower() < tostring(b):lower() end)
            end
            self.built:Fire()
        end

        function Catalog:SetIncludedTypes(types)
            self._typeSet = {}
            for _, t in ipairs(types or {}) do self._typeSet[t] = true end
        end

        function Catalog:GetIncludedTypes()
            local out = {}
            for t in pairs(self._typeSet) do out[t] = true end
            return out
        end

        function Catalog:SetIncludedRarities(rarities)
            self._raritySet = {}
            for _, r in ipairs(rarities or {}) do self._raritySet[r] = true end
        end

        function Catalog:GetIncludedRarities()
            local out = {}
            for r in pairs(self._raritySet) do out[r] = true end
            return out
        end

        function Catalog:IsInCatalog(opts)
            opts = opts or {}
            local itemName, cosmeticType, cosmeticName = opts.itemName, opts.cosmeticType, opts.cosmeticName
            if cosmeticType == "Skin" then
                local lookup = self._lookupByType.Skin[itemName]
                return lookup and lookup[cosmeticName] == true
            end
            return self._lookupByType[cosmeticType] and self._lookupByType[cosmeticType][cosmeticName] == true
        end

        function Catalog:IsEmpty(itemName, cosmeticType)
            if cosmeticType == "Skin" then
                local lookup = self._lookupByType.Skin[itemName]
                return not lookup or next(lookup) == nil
            end
            local lookup = self._lookupByType[cosmeticType]
            return not lookup or next(lookup) == nil
        end

        function Catalog:IsCosmeticType(name, t)
            return self._typeByName[name] == t
        end
        ...
        local function cosmeticTab(parent, title, list)
            local tab = parent:newTab(title):intoSections()
            local section = tab:newSection("left", title)
            local options = copyList(list)
            if #options == 0 then options = {"No items found"} end
            local dropdown = section:newDropdown("cosmetics/" .. string.lower(title) .. "/selected", false, options):set(options[1])
            return tab, dropdown
        end
        ...
        do
            ...
            info:newLabel("cosmetics/catalog/source"):setLabel("Source: CosmeticLibrary")

            local refresh = base.features["cosmetics/catalog/refresh"]
            if refresh then
                refresh.changed:Connect(function()
                    catalog:Build()
                    -- Re-apply current options to the category dropdowns.
                    for _, entry in ipairs({
                        {"cosmetics/skin/selected", catalog._byType.Skin},
                        {"cosmetics/wrap/selected", catalog._byType.Wrap},
                        {"cosmetics/charm/selected", catalog._byType.Charm},
                        {"cosmetics/finisher/selected", catalog._byType.Finisher},
                    }) do
                        local feature = base.features[entry[1]]
                        if feature then
                            for _, oldOption in ipairs(feature.options or {}) do feature:remove(oldOption) end
                            local opts = copyList(entry[2])
                            if #opts == 0 then opts = {"No items found"} end
                            for _, option in ipairs(opts) do feature:add(option) end
                            feature:set(opts[1])
                        end
                    end
                end)
        ...
        do
            -- Skin keeps the supplied catalog's item-name-aware lookup model.
            local skinTab = cosmeticTabs:newTab("Skin"):intoSections()
            local section = skinTab:newSection("left", "Skin")
            local itemNames = {}
            local seen = {}
            local lib = catalog:_getLibrary()
            local cosmeticsTable = lib and lib.Cosmetics
            if type(cosmeticsTable) == "table" then
                for name, data in pairs(cosmeticsTable) do
                    if type(data) == "table" and rawget(data, "Type") == "Skin" then
                        local itemName = rawget(data, "ItemName")
                        if itemName and not seen[itemName] then seen[itemName] = true; table.insert(itemNames, itemName) end
                    end
                end
            end
            ...
            local skinNames = {}
            if itemNames[1] ~= "No items found" then
                local lookup = catalog._lookupByType.Skin[itemNames[1]] or {}
                for name in pairs(lookup) do table.insert(skinNames, name) end
                table.sort(skinNames, function(a,b) return tostring(a):lower() < tostring(b):lower() end)
            end
            if #skinNames == 0 then skinNames = {"No skins found"} end
            local skinSelect = section:newDropdown("cosmetics/skin/selected", false, skinNames):set(skinNames[1])
            skinItem.changed:Connect(function(itemName)
                local lookup = catalog._lookupByType.Skin[itemName] or {}
                for _, oldOption in ipairs(skinSelect.options or {}) do skinSelect:remove(oldOption) end
                local opts = {}
                for name in pairs(lookup) do table.insert(opts, name) end
                table.sort(opts, function(a,b) return tostring(a):lower() < tostring(b):lower() end)
                if #opts == 0 then opts = {"No skins found"} end
                for _, option in ipairs(opts) do skinSelect:add(option) end
                skinSelect:set(opts[1])
            end)
        end

        cosmeticTab(cosmeticTabs, "Wrap", catalog._byType.Wrap)
        cosmeticTab(cosmeticTabs, "Charm", catalog._byType.Charm)
        cosmeticTab(cosmeticTabs, "Finisher", catalog._byType.Finisher)
