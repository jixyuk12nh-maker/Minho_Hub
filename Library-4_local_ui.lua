...
do
    ...
    function CustomImageManager.DownloadAsset(AssetName: string, ForceRedownload: boolean?)
        local AssetData = CustomImageManagerAssets[AssetName]
        if not AssetData or not AssetData.URL then
            return false, "no remote asset URL"
        end

        -- Built-in Obsidian assets are intentionally not downloaded from GitHub.
        if tostring(AssetData.URL):find("github.com", 1, true)
            or tostring(AssetData.URL):find("raw.githubusercontent.com", 1, true) then
            return false, "GitHub asset download disabled"
        end
...
--// Online Lucide icons disabled: no GitHub UI/icon loading.
